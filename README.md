...
- **Read side**: projections fold events into ordinary PocketBase collections, so the stock REST API, realtime subscriptions, auth rules and admin UI serve queries unchanged.
...
- **Functions (FaaS)**: user-defined functions via a runtime-agnostic interface; goja (JS) adapter first, with event / HTTP / cron triggers.
